...
def fun(x,y,a,b):
    ...
    return (a*math.log(y,2)) - (b*math.log(x,2))
...
#3x3
#KA
KKA = np.linspace(1,18,18)

#KB
KKB = np.linspace(1,18,18)
...
for i in range(len(KKA)):
    for j in range(len(KKB)):
        #print inI
        Fi[i,j] = fun(KKB[j], KKA[i],0.017,0.017)
        #Fi[i,j] = fun(KKB[j], KKA[i],1,1)
        print Fi[i,j], KKB[j], KKA[i]


plt.contourf(X, Y, Fi, 18, cmap="RdBu_r")
plt.colorbar()
